...
int main(void) {
	...
	ofstream file("RLC.dat");
	// Insert the file header here
	file << setw(10) << "Time" << setw(10) << "Voltage" << setw(10) << "Inductor" << setw(10) << "Capacitor" << setw(10) << "Resistor" << setw(10) << "Total" << endl;
	...
	for (double time = 0.0; time < 0.1; time += delta) {//the simulation runs for 0.1 seconds
		...
		file << fixed << setprecision(4) << setw(10) << time << setw(10) << voltage << setw(10) << currentInductor << setw(10) << currentCapacitor << setw(10) << currentResistor << setw(10) << totalCurrent << endl;
	}
	//Clean up here
	file.close();
